...
public enum TurboQuantRotation {
    ...
    public static func whtSigns(dim: Int, seed: UInt64) -> MLXArray {
        ...
        let signs = MLX.where(uniform .> 0.5, MLXArray(1.0).asType(uniform.dtype), MLXArray(-1.0).asType(uniform.dtype))
        eval(signs)
        return signs
    }
...
public class TurboQuantKVCache: BaseKVCache {
    ...
    public func compressedAttention(
        ...
        if rawKeyMode {
            ...
            // Use bare Float scalar (not MLXArray(Float)) to stay in bf16.
            ...
            case .array(let maskArray):
                if maskArray.dtype == .bool {
                    scores = MLX.where(maskArray, scores, MLXArray(Float.leastNormalMagnitude).asType(scores.dtype))
            ...
            case .causal:
                ...
                scores = MLX.where(expandedMask, scores, MLXArray(Float.leastNormalMagnitude).asType(scores.dtype))
            case .none: break
            default: break
            }
