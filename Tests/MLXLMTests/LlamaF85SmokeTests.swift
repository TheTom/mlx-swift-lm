...
//
// F-85 batched sparse decode — Llama smoke tests.
//
// Validates that the new Llama fullyBatchedSparseDecode path:
//   1. compiles + links against BatchedRetrievalAttentionKVCache
//   2. produces logits of the expected shape ([B, 1, vocab])
//   3. doesn't NaN on a synthetic random-weights LlamaModel
//
// A gated live smoke (RUN_LLAMA_F85_SMOKE=1) loads Llama-3.2-3B-4bit
...
@Suite("Llama F-85 batched sparse decode — port", .serialized)
struct LlamaF85SmokeTests {

    fileprivate static func makeSyntheticLlamaConfig() throws -> LlamaConfiguration {
    ...
    @Test("Llama fullyBatchedSparseDecode runs end-to-end on synthetic config")
    func llamaFullyBatchedSparseDecodeSynthetic() throws {
        let cfg = try LlamaF85SmokeTests.makeSyntheticLlamaConfig()
        let model = LlamaModel(cfg)

        // Build batched RA caches. Synthetic test on .float32 so we
        // don't trip the bf16 upcast.
        let B = 2
        let T = 256
        let nKVH = 2
        let dHead = 8
        let nLayers = 2
        let raCaches = LlamaF85SmokeTests.makeBatchedRACaches(
        ...
        // Shape: [B, 1, vocab] — vocab from LlamaModel.vocabularySize
        // (the only public surface; LlamaConfiguration's field is
        // internal). Synthetic config sets vocab=128.
        let V = model.vocabularySize
        #expect(logits.shape == [B, 1, V],
            "expected logits [B=\(B), 1, V=\(V)], got \(logits.shape)")

        // Finite check — random-weight Llama produces finite output.
    ...
    /// Gated real-model smoke. Loads Llama-3.2-3B-Instruct-4bit, builds
    ...
    /// generation quality.
    ///   RUN_LLAMA_F85_SMOKE=1 swift test --filter llamaF85LiveSmoke
    @Test func llamaF85LiveSmoke() throws {
        guard ProcessInfo.processInfo.environment["RUN_LLAMA_F85_SMOKE"] == "1" else {
            return
        }
        ...
        print("[llama-f85] loading...", flush: true)
        let configData = try Data(contentsOf: modelPath.appendingPathComponent("config.json"))
        let cfg = try JSONDecoder().decode(LlamaConfiguration.self, from: configData)
        let model = LlamaModel(cfg)
        try loadWeights(
            modelDirectory: modelPath, model: model,
            quantization: BaseConfiguration.Quantization(groupSize: 64, bits: 4))
        print("[llama-f85] loaded", flush: true)

        // Read shape fields from the raw JSON since LlamaConfiguration's
        // fields are internal to MLXLLM and the tests live in their own
        // module. Matches the Qwen2 smoke test pattern.
        struct LlamaShape: Codable {
            let hidden_size: Int
        ...
        let shape = try JSONDecoder().decode(LlamaShape.self, from: configData)
        let resolvedHeadDim = shape.head_dim ?? (shape.hidden_size / shape.num_attention_heads)

        // B=4 ctx=8K matches the Qwen3 F-85 smoke spec.
        let B = 4
        let T = 8 * 1024
        ...
        print("[llama-f85] B=\(B) T=\(T) nLayers=\(nLayers) nKVH=\(nKVH) D=\(dHead)",
              flush: true)
        ...
        print("[llama-f85] one batched sparse decode step: \(Int((t1 - t0) * 1000))ms "
        ...
        print("[llama-f85] smoke done", flush: true)
    }
}
